...
import ReceivableFifoTable from '@/containers/Receivables/ReceivableFifoTable';
...
const TAG = "ReceivableFifo: ";
const ReceivableFifo = (props: any) => {
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.receivableFifo}?company=${company}&ledger=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.receivableFifo}?company=${company}&&ledger=${ledgerTarget}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
    ...
    NetworkOps.makeGetRequest(apiUrl, true)
      .then(async (response: any) => {
        ...
        if (response?.status == 200) {
          setDataList(response?.data?.data?.data ? response?.data?.data?.data : []);
