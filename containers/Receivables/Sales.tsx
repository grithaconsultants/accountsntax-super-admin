...
import SalesItemTable from '@/containers/Receivables/SalesItemTable';
...
const Sales = (props: any) => {
  ...
  function onFilterClick() {
    ...
    if (initialPoint === endPoint) {
      ...
      // setLoading(true);
      const docElement: any = document.getElementById("searchText07");
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.salesRegister}?company=${company}&ledger=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.salesRegister}?company=${company}&ledger=${ledgerTarget}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
