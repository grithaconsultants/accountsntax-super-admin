...
import SalesReceiptTable from '@/containers/Receivables/SalesReceiptTable';
...
const SalesReceipt = (props: any) => {
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.salesReceipt}?company=${company}&ledger=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.salesReceipt}?company=${company}&&ledger=${ledgerTarget}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
