...
import SalesItemwiseTable from '@/containers/Mis/SalesItemwiseTable';
...
const SalesItemwise = () => {
  ...
  function onFilterClick() {
    ...
    if (initialPoint === endPoint) {
      ...
      // setLoading(true);
      const docElement: any = document.getElementById("searchText02");
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.salesItemwise}?company=${company}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.salesItemwise}?company=${company}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
      ...
      <div className="d-flex justify-content-between pb-5 align-items-end" >
        ...
        <div className='d-flex' >
          ...
          {dataList.length ?
            <div className="ms-3 d-flex mb-2 align-items-end pb-1 " >
              <IconDropdown
                sheetName="sales-item-wise"
                fileName="sales-item-wise.xlsx"
                sheetData={dataList}
                isXLSX={true}
                isCSV={false}
                isPDF={false}
