...
import SalesRegisterTable from '@/containers/Mis/SalesRegisterTable';
...
const SalesRegister = () => {
  ...
  function onFilterClick() {
    ...
    if (initialPoint === endPoint) {
      ...
      // setLoading(true);
      const docElement: any = document.getElementById("searchText03");
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.salesRegister}?company=${company}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.salesRegister}?company=${company}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
