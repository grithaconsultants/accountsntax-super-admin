...
import SalesTable from '@/containers/SalesTable/SalesTable';
...
const SalesCustomerwise = () => {
  ...
  useEffect(() => {
    if (company !== null) {
      ApicallForData(defaultCurrent, defaultPageSize, company, sortKey, sortType, initialPoint, endPoint, searchString);
    }
  ...
  function onFilterClick() {
    ...
    if (initialPoint === endPoint) {
      ...
      // setLoading(true);
      const docElement: any = document.getElementById("searchText01");
      ...
      // console.log(' onFilterClick got call ', initialPoint, endPoint, searchItem);
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.salesCustomerwise}?company=${company}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.salesCustomerwise}?company=${company}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
