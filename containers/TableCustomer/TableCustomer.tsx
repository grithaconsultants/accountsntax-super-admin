        ...
        <thead>
          <tr>
            ...
            <th className="ps-4" >
              Party Name
              <Image
                src={sortKey === "partyName" && sortType == "asc" ? upar : uparin}
                alt="table arrow"
                width={15}
                height={20}
                onClick={filterTable.bind('', 'partyName', 'asc')}
              />
              <Image
                src={sortKey === "partyName" && sortType == "desc" ? downar : downarin}
                alt="table arrow"
                width={15}
                height={20}
                onClick={filterTable.bind('', 'partyName', 'dsc')}
              />
            </th>
            <th>GSTIN/UIN
              {/* <Image src={sortKey === "gstin" && sortType == "asc" ? upar : uparin} alt="table arrow" width={15} height={20} onClick={filterTable.bind('', 'gstin', 'asc')} />
              <Image src={sortKey === "gstin" && sortType == "desc" ? downar : downarin} alt="table arrow" width={15} height={20} onClick={filterTable.bind('', 'gstin', 'dsc')} /> */}
            </th>
            ...
            return (
              <tr key={index} >
                ...
                <td className="tb-text tb-mw-150 px-1 tb-w-max" >
                  <CustomTooltip placement="topLeft" title={item?.mst_ledgers?.mobile} > {item?.mst_ledgers?.mobile}</CustomTooltip>
                </td>
                <td className="tb-text tb-mw-150 px-1" >
                  <CustomTooltip placement="topLeft" title={item?.mst_ledgers?.email} > {item?.mst_ledgers?.email}</CustomTooltip>
                </td>
