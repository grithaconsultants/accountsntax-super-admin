...
import { removeDateRest, yyyymmTommmYY, decimalTwo, wait, SrPageNumber } from '@/utils/helper';

const TAG = "ExpenseRegisterTable: ";
const ExpenseRegisterTable = (props: any) => {
  ...
  async function toReceivable(item: any) {
    ...
    localStorage.setItem('payableParty', item);
    await wait(200);
          ...
          {rowsDataList.map((item: any, index: number) => {
            return (
              <tr key={index} >
                ...
                <td className="tb-text tb-mw-150 px-1" >
                  <CustomTooltip placement="topLeft" title={item?.trn_vouchers?.party_name} >
                    <span className='tx-o cp' onClick={toReceivable.bind('', item?.trn_vouchers?.party_name)} >
                      {item?.trn_vouchers?.party_name}
                    </span></CustomTooltip>
                </td>
...
export default ExpenseRegisterTable;