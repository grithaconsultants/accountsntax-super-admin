...
import CategoryWiseTable from '@/containers/Expenses/CategoryWiseTable';
...
const TAG = "CategoryWise: ";
const CategoryWise = (props: any) => {
  ...
  function onFilterClick() {
    ...
    if (initialPoint === endPoint) {
      ...
      // setLoading(true);
      const docElement: any = document.getElementById("searchText05");
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.categoryWise}?company=${company}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.categoryWise}?company=${company}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
    ...
    NetworkOps.makeGetRequest(apiUrl, true)
      .then(async (response: any) => {
        ...
        if (response?.status == 200) {
          setDataList(response?.data?.data[0]?.expenses ? response?.data?.data[0]?.expenses : []);
