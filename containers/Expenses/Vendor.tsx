...
import VendorWiseTable from '@/containers/Expenses/VendorTable';
...
const TAG = "VendorWise: ";
const VendorWise = (props: any) => {
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.expensePerformance}?company=${company}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.expensePerformance}?company=${company}&search=${search}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
