...
import ExpensesTable from '@/containers/Payables/ExpensesTable';
...
const TAG = "ExpensesComp: ";
const ExpensesComp = (props: any) => {
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.payablesExpenses}?company=${company}&search=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.payablesExpenses}?company=${company}&search=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
