...
import PaymentsTable from '@/containers/Payables/PaymentsTable';
...
const TAG = "PaymentsComp: ";
const PaymentsComp = (props: any) => {
  ...
  async function ApicallForData(page: any, limit: any, company: any, sortKey: any, sortType: any, startDate: any, endDate: any, search: any): Promise<void> {
    ...
    if (isEmpty(search)) {
      apiUrl = `${endPoints.payablesPayments}?company=${company}&ledger=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    } else {
      apiUrl = `${endPoints.payablesPayments}?company=${company}&ledger=${ledgerTarget}&fromDate=${startDate}&toDate=${endDate}&page=${page}&limit=${limit}`;
    }
